...
namespace StockRoom11net.Controls.TabControl.Control
{
    ...
    public class FATabStrip : BaseStyledPanel, ISupportInitialize, IDisposable
    {
        ...
        [Category("TabControls Properties")]
        ...
        {
            if (!menu.Visible && menu.Items.Count > 0)
            {
                if (RightToLeft == RightToLeft.No)
                {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // Draw a rectangle around the whole control. 
            ...
            if (RightToLeft == RightToLeft.No)
            {
                ...
                {
                    e.Graphics.DrawLine(SystemPens.ControlDark, new Point(0, UnderPageLine),
                                        new Point(ClientRectangle.Width, UnderPageLine));
                }
                else if (SelectedItem != null && SelectedItem.IsDrawn)
                {
                    Point end = new Point((int)SelectedItem.StripRect.Left - 9, UnderPageLine);
                    e.Graphics.DrawLine(SystemPens.ControlDark, new Point(0, UnderPageLine), end);
                    end.X += (int)SelectedItem.StripRect.Width + 10;
                    e.Graphics.DrawLine(SystemPens.ControlDark, end, new Point(ClientRectangle.Width, UnderPageLine));
                }
            }
            else
            {
                ...
                {
                    Point end = new Point((int)SelectedItem.StripRect.Left, UnderPageLine);
                    e.Graphics.DrawLine(SystemPens.ControlDark, new Point(0, UnderPageLine), end);
        ...
        private void UpdateLayout()
        {
            ...
            if (_alignment == Alignment.Top)
            {
                ...
                UnderPageLine = DEF_HEADER_HEIGHT;

                if (RightToLeft == RightToLeft.No)
                {
                    stripButtonRect = new Rectangle(0, 0, ClientSize.Width - DEF_GLYPH_WIDTH - 2, 10);
                    menuGlyph.Bounds = new Rectangle(stripButtonRect.Right + 0, 2, 16, 16);
                ...
                else
                {
                    ...
                    menuGlyph.Bounds = new Rectangle(20 + 4, 2, 16, 16); //ClientSize.Width - 20, 2, 16, 16);
                }
            ...
            {
                DockPadding.Top = 1;
                DockPadding.Bottom = DEF_HEADER_HEIGHT + 1;
                DockPadding.Right = 1;
                DockPadding.Left = 1;

                UnderPageLine = ClientSize.Height - DEF_HEADER_HEIGHT - 1;

                if (RightToLeft == RightToLeft.No)
                {
                    stripButtonRect = new Rectangle(0, ClientSize.Height - DockPadding.Bottom, ClientSize.Width - DEF_GLYPH_WIDTH - 2, ClientSize.Height - 2);
                    menuGlyph.Bounds = new Rectangle(ClientSize.Width - DEF_GLYPH_WIDTH + 2, ClientSize.Height - DEF_HEADER_HEIGHT + 1, 16, 16);
                    closeButton.Bounds = new Rectangle(ClientSize.Width - 20, ClientSize.Height - DEF_HEADER_HEIGHT + 1, 16, 16);
                }
                else
                {
                    stripButtonRect = new Rectangle(DEF_GLYPH_WIDTH + 2, ClientSize.Height - DockPadding.Bottom, ClientSize.Width - DEF_GLYPH_WIDTH - 15, ClientSize.Height - 2);
                    closeButton.Bounds = new Rectangle(4, ClientSize.Height - DEF_HEADER_HEIGHT + 1, 16, 16);
                    menuGlyph.Bounds = new Rectangle(22, ClientSize.Height - DEF_HEADER_HEIGHT + 1, 16, 16);
                }
            }
        }
