...
namespace StockRoom11net.Controls
{
    ...
    public class CurrentStatus
    {
        ...
        public CurrentStatus(DataGridViewRow currentDataGridViewRow)
        {
            try
            {
                ...
                // It's a Grouping source, have not status column......
                if (currentDataGridViewRow.Index == -1 || currentDataGridViewRow.DataBoundItem.GetType() != typeof(DataRowView))
                {
                    SetProperties(StatusDefaultValue);
                    return;
                }
                ...
                InitializeStatus(_currentDataRowView.Row);
            }
            catch (Exception error)
            {
                ColStatusIndex = -1;
        ...
        void InitializeStatus(DataRow rowDataRow)
        {
            ...
            if (_rowDataRow.Table.Columns.Contains(@"PartNumber"))
            {
                ...
                IsBOM = Utilities.IsPartNumberBOM(PartNumber);
            }
            ...
            if (_rowDataRow.Table.Columns.Contains("Properties"))
            {
                ...
                foreach (string propertyItem in propertyItems)
                {
                    ...
                    if (propertyItem.Contains("Date:"))
                    {
                        ...
                        if (propertyItem.Contains("BOM"))
                        {
                            StateInformationObj.State = Utilities.StateItemsData.None;
                    ...
                    if (IsBOM)
                    {
                        StateInformationObj.State = Utilities.StateItemsData.UpDate;
                        BOMInformationObj = new BOMInformation(BOMInformationObj.DefaultBOMInformation);
            ...
            if (_rowDataRow.Table.Columns.Contains("CountPDF"))
            {
                ...
                HeaderInformationObj.CountPDF = Utilities.CastAsInt(_rowDataRow.ItemArray[ColCountPDFIndex]);
            }
            else
            ...
            if (_rowDataRow.Table.Columns.Contains("CountDoc"))
            {
                ...
                HeaderInformationObj.CountDoc = Utilities.CastAsInt(_rowDataRow.ItemArray[ColCountDocIndex]);
            }
            else
            ...
            if (_rowDataRow.Table.Columns.Contains("CountTxT"))
            {
                ...
                HeaderInformationObj.CountTxT = Utilities.CastAsInt(_rowDataRow.ItemArray[ColCountTxTIndex]);
            }
            else
            ...
            if (_rowDataRow.Table.Columns.Contains("CountDocx"))
            {
                ...
                HeaderInformationObj.CountDocx = Utilities.CastAsInt(_rowDataRow.ItemArray[ColCountDocxIndex]);
            }
            else
        ...
        public bool IsMarkWithNote
        {
            get
            {
                if (Note == null || (Note.Length == 4 && Note.Contains("Null")))
                    return false;
        ...
        public bool HasNote
        {
            get
            {
                if (Note == null || Note.Contains("Null"))
                    return false;
        ...
        void SetProperties(string nameValueProperties)
        {
            try
            {
                ...
                if (_strings.Length >= 2 && _strings[0].Contains("Locked"))
                    Locked = Convert.ToBoolean(_strings[1]);
                else
                    SetPropertiesErrorCatch();

                if (_strings.Length >= 4 && _strings[2].Contains("Selected"))
                    Selected = Convert.ToBoolean(_strings[3]);
                else
                    SetPropertiesErrorCatch();

                if (_strings.Length >= 6 && _strings[4].Contains("Unerasable"))
                ...
                if (_strings.Length >= 10 && _strings[8].Contains("Note"))
                    Note = _strings[9];
                else
                    SetPropertiesErrorCatch();
            ...
            catch(Exception error)
            {
                ...
                {
                    MessageBox.Show(form, @"Message related to this error is " + error.Message,
                                          @"DataGridViewExtended has generated an error at SetProperties(string nameValueProperties)",
                                          MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
    ...
    public class HeaderInformation
    {
        ...
        public string HeaderInfContent
        {
            get
            {
                ...
                string headerInfContent = builder.ToString();

                return headerInfContent.ReplaceLast(",", "");
            }
            set
            {
    ...
    public class StateInformation
    {
        ...
        {
            _state = Utilities.StateItemsData.None;
        ...
        Utilities.StateItemsData _state;
        public Utilities.StateItemsData State
        {
            get
            {
                return _state;
