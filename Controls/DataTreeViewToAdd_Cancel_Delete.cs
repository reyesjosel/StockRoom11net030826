...
namespace StockRoom11net.Controls
{
    public partial class DataTreeViewToAddCancelDelete : UserControl
    {
        ...
        public BindingSourceValidating<Table_Base_TreeView> BindingSourceTreeView
        {
            ...
            set
            {
                try
                {
                    ...
                    var validIDs = new HashSet<int>(items.Select(n => n.ID))
                                                {
                    ...
                    OrphansNodes = items.Where(n => n.Parent_ID == null   // ← null = unattached node, no parent reference at all
                                                 || n.Parent_ID is > 0 
                    ...
                    Debug.WriteLine($"Total: {items.Count} | Roots (Parent_ID==0): {roots.Count} | DupIDs: {string.Join(",", dupIDs)} | Orphans: {OrphansNodes.Count}");
       ...
       // public string CurrentDepartmentLogIn { get; set; }
            ...
            try
            {
                ...
                if (DataBoundObject_Name.Contains("TreeView"))
                {
                    ...
                    if (_currentNodeToAddTBT != null)
                        if (_currentNodeItem.ID == _currentNodeToAddTBT.ID)
                            return;
            ...
            try
            {
                ...
                if (DataBoundObject_Name.Contains("TreeView"))
                {
                    ...
                    if (_currentNodeToDeleteTBT != null)
                        if (_currentNodeItem.ID == _currentNodeToDeleteTBT.ID)
                            return;
        ...
        async void ToolStripMenuItem_DeletedAllNodes_Click(object? sender, EventArgs e)
        {
            try
            {
                if (olvDataTree_ToDelete.SelectedItem == null)
                    ...
                    DialogResult dialogResult =
                    MessageBox.Show("This action will delete all ( " + OrphansNodes.Count + " ) nodes and children.\r\n " +
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        // ✅ Before: 2× DB round-trips per node (FindAsync + SaveChangesAsync in a loop)
                        // ✅ After:  1 SQL statement for ALL nodes — DELETE WHERE Index IN (...)
                        var indexes = OrphansNodes.Select(n => n.Index).ToList();

                        if (TableName.Contains("Table_StockRoom_TreeView"))
                            await _unitOfWork.TableStockRoomTreeViewRepository.DeleteRangeAsync(indexes);
                        else if (TableName.Contains("Table_TimeLine_TreeView"))
                            await _unitOfWork.TableTimeLineTreeViewRepository.DeleteRangeAsync(indexes);

                        // Remove from BindingSource in memory (no DB calls)
                        foreach (Table_Base_TreeView itemEF in OrphansNodes)
                            RemoveFromBindingSourceByIndex(itemEF.Index);
        ...
        /// </summary>
        private void RemoveFromBindingSourceByIndex(int index)
        {
            try
            {
