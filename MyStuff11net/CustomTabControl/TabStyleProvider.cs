...
namespace System.Windows.Forms
{
    ...
    public abstract class TabStyleProvider : Component
    {
        ...
        /// <summary>
        /// Retur a LinearGradientBrush used to fill the TabBackground.
        /// </summary>
        ...
        public Point Padding
        {
            ...
            set
            {
                ...
                if (_ShowTabCloser)
                {
                    ...
                    {
                        ((TabControl)_TabControl).Padding = new Drawing.Point(0, (int)value.Y);
                    }
                    else
                    {
                        ((TabControl)_TabControl).Padding = new Drawing.Point((int)value.X + (_Radius / 2) + 6, (int)value.Y);
                    }
                }
                else
                {
                    ...
                    {
                        ((TabControl)_TabControl).Padding = new Drawing.Point((int)value.X + (int)(_Radius / 2) - 1, (int)value.Y);
                    }
                }
            }
        }
            ...
            if (_FocusTrack && _TabControl.Focused && index == _TabControl.SelectedIndex)
            {
                ...
                switch (_TabControl.Alignment)
                {
                    case TabAlignment.Top:
                        ...
                        focusBrush = new LinearGradientBrush(focusRect, _FocusColor, Color.Black, LinearGradientMode.Vertical);
                        break;
                    case TabAlignment.Bottom:
                        focusRect = new Rectangle((int)pathRect.X, (int)pathRect.Bottom - 4, (int)pathRect.Width, 4);
                        focusBrush = new LinearGradientBrush(focusRect, Color.Black, _FocusColor, LinearGradientMode.Vertical);
                        break;
                    case TabAlignment.Left:
                        focusRect = new Rectangle((int)pathRect.X, (int)pathRect.Y, 4, (int)pathRect.Height);
                        focusBrush = new LinearGradientBrush(focusRect, _FocusColor, Color.Black, LinearGradientMode.Horizontal);
                        break;
                    case TabAlignment.Right:
                        focusRect = new Rectangle((int)pathRect.Right - 4, (int)pathRect.Y, 4, (int)pathRect.Height);
                        focusBrush = new LinearGradientBrush(focusRect, Color.Black, _FocusColor, LinearGradientMode.Horizontal);
                        break;
                }
        ...
        /// <summary>
        /// Return a SolidBrush with correct color used to fill PageBackground.
        /// </summary>
        /// <param name="index"></param>
        /// <returns></returns>
        public virtual Brush GetPageBackgroundBrush(int index)
        {
            ...
            if (_TabControl.SelectedIndex == index)
            {
                ...
                if (!_TabControl.TabPages[index].Enabled)
                {
                    light = Color.FromArgb(207, 207, 207);
                }
                else
                    if (_HotTrack && index == _TabControl.ActiveIndex)
                    {
                        //	Enable hot tracking
                        light = Color.FromArgb(234, 246, 253);
                    }
