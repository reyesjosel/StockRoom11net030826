...
public class Table_Base_TreeView : ITableBaseTreeView
{
    ...
    // Parent_ID is nullable: root-level tree nodes have no parent and store NULL in the database.
    // Do NOT add [Required] here — that would set IProperty.IsNullable=false in EF Core, causing
    // GetInt32 to be called without a DBNull guard and throwing InvalidOperationException on NULL rows.
    // NOTE: Do NOT default to a non-null value such as 75 (rootKeyValueToDelete). A non-null default
    //       means any unsaved new record would appear to be a child of that virtual root, and if the
    //       root node itself carried the same ID the tree would form a self-referencing cycle that
    //       causes Branch.Visible.get to recurse infinitely → StackOverflowException.
    public int? Parent_ID { get; set; } = null;

    
    // string? — column can be NULL in the database; EF Core will check IsDBNull before calling GetString.
    public string? Code { get; set; } = "";

    public string? Range { get; set; } = "";

    // string? — [Required] removed: that attribute sets IsNullable=false in EF Core, causing GetString to be
    // called without a DBNull guard and throwing InvalidOperationException when the DB column is NULL.
    ...
    // string? — [Required] removed: see Text_Name comment above.
    ...
    public bool IsDepartment
    {
        get
        {
            if (String_Filter != null && String_Filter.Contains("Department LIKE '*Department*'"))
                return true;
            else
                return false;
        }
