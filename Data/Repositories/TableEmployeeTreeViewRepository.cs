...
/// Repository interface for TableEmployeeTreeView operations
/// </summary>
public interface ITableEmployeeTreeViewRepository : IRepository<Table_Employees_TreeView>
{
    ...
    Task<IEnumerable<Table_Employees_TreeView>> GetTreeHierarchyAsync(int? rootId = null, CancellationToken cancellationToken = default);

    // Query operations
    Task<IEnumerable<Table_Employees_TreeView>> FindByCodeAsync(string code, CancellationToken cancellationToken = default);
    Task<IEnumerable<Table_Employees_TreeView>> FindByStatusAsync(string status, CancellationToken cancellationToken = default);
    Task<IEnumerable<Table_Employees_TreeView>> SearchByTextAsync(string searchTerm, CancellationToken cancellationToken = default);
...
/// <summary>
/// ✅ REPOSITORY — only data access, no logic
/// Repository implementation for TableEmployeeTreeView entity
/// </summary>
public class TableEmployeeTreeViewRepository : Repository<Table_Employees_TreeView>, ITableEmployeeTreeViewRepository
{
    public TableEmployeeTreeViewRepository(ProductionInventoryContext context) : base(context)
    {}
    ...
    /// <returns></returns>
    /// <exception cref="ArgumentNullException"></exception>
    public async Task<Table_Employees_TreeView> AddAsync(Table_Employees_TreeView entity, CancellationToken cancellationToken = default)
    {
        if (entity == null)
            throw new ArgumentNullException(nameof(entity));

        await _context.Table_Employees_TreeViews.AddAsync(entity, cancellationToken);
    ...
    public async Task<IEnumerable<Table_Employees_TreeView>> GetTreeHierarchyAsync(int? rootId = null, CancellationToken cancellationToken = default)
    {
        if (rootId.HasValue)
        {
    ...
    public async Task<IEnumerable<Table_Employees_TreeView>> FindByCodeAsync(string code, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(code))
            return Enumerable.Empty<Table_Employees_TreeView>();
    ...
    public async Task<IEnumerable<Table_Employees_TreeView>> FindByStatusAsync(string status, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IEnumerable<Table_Employees_TreeView>> SearchByTextAsync(string searchTerm, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(searchTerm))
            return Enumerable.Empty<Table_Employees_TreeView>();

        var term = searchTerm.ToLower();
        return await _context.Table_Employees_TreeViews.AsNoTracking()
            .AsNoTracking()
